...
var paperFalling = function (el) {
    ...
    this._resetPapers = function (paper) {
        //paper.x = this.width/2*Math.random()+(this.width)/2
        // place paper towards the far right third of canvas
        if (Math.random() > 0.5) {
            paper.x = 50 + this.width / 3 * Math.random(); //this.width /2 + Math.random() * (this.width / 3);
        }
        else {
            paper.x = (2 * this.width) / 3 + (this.width / 3) * Math.random() - 50;
        }
        paper.y = -50; //so paper is not visible on spawn
        ...
        paper.ySpeed = Math.random() * 0.5 + 0.35;
        return paper;
    };
